<template>
  <div class="center-main">
    ...
    <div class="account-rows clearFix">
      ...
      <a href="javascript:void (0);" class="link-btn set-state fl" @click="logoutConditions">查看注销条件</a>
    ...
    <div class="image-cut-controls" v-show="dialogHeadPortraitVisible">
      <ImageCutControls v-on:closeImageCutControls="closeImageCutControls" :photos="photos"
                        @sendEditAvatar="showEditAvatar"></ImageCutControls>
    </div>
    <div class="shade-div" v-show="dialogHeadPortraitVisible"></div>
    ...
    <el-dialog title="实名认证" custom-class="real-name-main" width="600px" :visible.sync="dialogRealNameVisible">
      <div class="realContent">
        <el-form :model="realName" label-width="100px">
          ...
          <el-form-item label="手持证件照" required class="uploadIdentification" v-if="typeStatus">
            <el-upload
              drag
              class="upload-demo"
              :action="upLoadImg"
    ...
    <el-dialog title="申请注销条件" custom-class="logout-model" width="496px" :visible.sync="dialogLogoutConsVisible">
      ...
      <footer slot="footer" class="dialog-footer">
        <el-button v-dbClick type="primary" @click="readAndApplyLoginOut">我已阅读</el-button>
      </footer>
    </el-dialog>
    <!--申请注销-->
    <el-dialog title="申请注销条件" custom-class="login-out-set" width="476px" :visible.sync="dialogLoginOutVisible">
...
<script type="text/babel">
  ...
  import ImageCutControls from "@/components/ImageCutControls";
  ...
  export default {
    data(){
      return {
        ...
        upLoadImg: baseUrl + "Common/upload?type=realname",
        certificateImageUrl: '',
    ...
    mounted(){
      ...
      if (window.localStorage.getItem("UserInfo")) {
        ...
        if (userNews.userinfo.avatar) {
          this.avatarPhoto = imgBaseUrl + userNews.userinfo.avatar;
        }
      }
      this.getUserInfo();
    },
    methods: {
      /*关闭头像设置框*/
      closeImageCutControls(res){
        this.dialogHeadPortraitVisible = false;
        if (res) {
          this.avatarPhoto = imgBaseUrl + res.photos || "";
        }
      },
          ...
          this.$axios(config).then(res => {
            if (res.data.code) {
              ...
              if (res.data.data.avatar) {
                this.avatarPhoto = imgBaseUrl + res.data.data.avatar;
...
//        _this.sendMsgDisabled = true;
      ...
      readAndApplyLoginOut(){
        this.dialogLogoutConsVisible = false
      }
    },
    components: {
      LoginOut,
