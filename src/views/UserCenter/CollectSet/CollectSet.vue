<template>
  <div class="center-main">
    ...
    <div v-if="type" class="collect-rows clearFix">
      ...
      <a href="javascript:void(0);" class="set-state fl">
        <span style="float: left">{{alipayData.card || '未设置'}}</span>
        <div class="dig" >
          <img v-if="alipayData.card" :src="alipayData.img" alt="" style="width: 100%;height: 100%">
        </div>
        <span v-if="alipayData.card" style="float: left;margin-left: 20px;">审核状态：{{statusTest}}</span>
      </a>
        ...
        try {
          this.$axios({
            ...
            if (until.responesValidate(result)) {
              ...
              if(self.alipayData.check_status == '1'){
                self.statusTest = '审核中'
              } else if (self.alipayData.check_status == '2'){
                self.statusTest = '已审核'
              } else if (self.alipayData.check_status == '3'){
                self.statusTest = '审核失败'
              } else {
                self.statusTest = '暂无'
              }
            }
