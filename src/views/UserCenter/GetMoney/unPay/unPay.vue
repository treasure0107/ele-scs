      ...
      <el-table
        ...
        <el-table-column
          ...
          <!--<template slot-scope="scope">-->
            <!--<router-link style="color:#5357E2;" :to="'/unPayDetails?id='+scope.row.id">{{scope.row.trade_no}}-->
            <!--</router-link>-->
          <!--</template>-->
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          fit=true>
          <template slot-scope="scope">
            <CountDown :timeCountdown="scope.row.daojishi" @fnTimeOver="countdown(scope.$index)"></CountDown>
          </template>
        </el-table-column>
        <el-table-column
          label="操作"
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      ...
      getData(){
        try {
          let url = baseUrl + "/account/Exchange/getTxNoPayList";
          let config = {
            method: "POST",
      ...
      handleClickUnPayDetails(id) {  //详情
        this.$router.push('/GetMoneyUnPayDetails?id=' + id);
      },
      countdown(row){
//        console.log(row)
        /* var n = row.$index
