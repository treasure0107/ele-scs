          ...
          <div class="orderMain">
            <div class="order-info-news">
              <el-row>
                ...
                <el-col :span="24" v-if="payment_money">
                ...
                <el-col :span="24">
                  ...
                  <div class="con">
                    ...
                    <p style="display: none;font-size: 14px" id="pp">败诉原因：由于您上传的凭证不真实，无法证实你已完成转账。</p>
                    <p style="display: none;font-size: 14px" id="qq">交易数字资产将会重新匹配买家</p>
                    <p style="display: none;font-size: 14px" id="bb">你的订单{{trade_no}}因付款超时已被系统取消</p>
...
<script type="text/babel">
  ...
  export default {
    ...
    mounted(){
      ...
      if (buySel==0) {
        this.getCelDetailsIn();
      } else {
        this.getCelDetailsOut();
      }
    },
    methods: {
      getCelDetailsIn(){    //买入
...
//        let id=this.$route.params.id;
        try {
          let url = baseUrl + "account/trade_detail/cancel_in";
          let config = {
            method: "POST",
          ...
          this.$axios(config).then(res => {
            if (until.responesValidate(res)) {
              ...
              if (!dataObj.is_appeal && !dataObj.overtime) {
                document.getElementById('img1').style.display='block';
                document.getElementById('img2').style.display='none';
                document.getElementById('img3').style.display='none';
                this.test = '订单已取消，付款参考号：'+ dataObj.re_num
                this.sr = '';
                this.sc = '';
              } else if (dataObj.is_appeal && !dataObj.overtime) {
...
//        let id=this.$route.params.id;
        try {
          let url = baseUrl + "account/trade_detail/cancel_out";
          let config = {
            method: "POST",
            url: url + "?r=" + Math.random(),
            data: {
