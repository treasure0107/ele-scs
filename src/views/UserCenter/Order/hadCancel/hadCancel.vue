    ...
    <div class="tab-list">
      <el-table
        ...
        <el-table-column
          ...
          align="center">
          <template slot-scope="scope">
            <span v-text="dateFn(scope.row.cancel_time)"></span>
          </template>
        </el-table-column>
        <el-table-column
          label="操作"
  ...
  export default {
    ...
    methods: {
      getAccountFipaidIn(){    //买入
        try {
          let url = baseUrl + "account/trade_list/cancel_in";
          let config = {
            method: "POST",
      ...
      getAccountFipaidOut(){    //卖出
        try {
          let url = baseUrl + "account/trade_list/cancel_out";
          let config = {
            method: "POST",
            url: url + "?r=" + Math.random(),
            data: {
      ...
      handleClickfiPayDetails(id) {  //详情
        this.$router.push('/hadCancelDetails?id=' + id + '&buySel=' + this.buySel);
      },
      handleSizeChange(){
